...
class ReachGoalBasic(primitive.Primitive):
    ...
    def __init__(self, maxMove=0.05, maxRotation=math.pi/20,
        ...
        super(ReachGoalBasic, self).__init__(*args, **kwargs)
    ...
    def _getReward(self):
        ...
        if self.sparse:
            ...
            # accuracyReward = math.exp(-error)
            #
            # # Precision reward
            # nearReward = 0
            # if error < 0.2:
            #     nearReward = math.exp(10*-error)
            #
            # # Precision reward
            # precisionReward = 0
            # if error < 0.1:
            #     precisionReward = math.exp(25*-error)
            #
            # reward = accuracyReward + nearReward + precisionReward

            # reward = math.exp(-error) + math.exp(10*-error) + math.exp(25*-error)
            reward = math.exp(2*-error)
            # reward = accuracyReward + successReward
