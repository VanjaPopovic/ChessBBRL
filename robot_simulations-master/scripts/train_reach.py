...
if __name__ == '__main__':
    ...
    def evaluate_policy(model, env, n_eval_episodes=10, deterministic=True,
        ...
        for _ in range(n_eval_episodes):
            ...
            info = info[0]
            episode_dists.append(info['goal distance'])
        ...
        if return_episode_rewards:
            return episode_rewards, episode_lengths, episode_dists, episode_collisions, episode_success
    ...
    class EvalCallback(EventCallback):
        ...
        def _on_step(self) -> bool:
            ...
            if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
                ...
                episode_rewards, episode_lengths, episode_dist, episode_collisions, episode_success = evaluate_policy(self.model, self.eval_env,
                ...
                summary = tf.Summary(value=[tf.Summary.Value(tag='eval/mean_final_goal_dist', simple_value=np.mean(episode_dist))])
                self.locals['writer'].add_summary(summary, self.num_timesteps)
                summary = tf.Summary(value=[tf.Summary.Value(tag='eval/mean_episode_collisions', simple_value=np.mean(episode_collisions))])
                self.locals['writer'].add_summary(summary, self.num_timesteps)
                summary = tf.Summary(value=[tf.Summary.Value(tag='eval/mean_episode_success', simple_value=np.mean(episode_success))])
                self.locals['writer'].add_summary(summary, self.num_timesteps)

            return True

    eval_env = DummyVecEnv([lambda: gym.make("Reach-Ur103f-v0")])
    ...
    if algoName == "PPO":
        eval_callback = EvalCallback(eval_env, best_model_save_path=logPath, n_eval_episodes=10,
                                     log_path=logPath, eval_freq=100, deterministic=True, render=False, training_env=env)
        # eval_callback = EvalCallback(eval_env, best_model_save_path=logPath, n_eval_episodes=10,
        ...
        class CustomPolicy(FeedForwardPolicy):
            def __init__(self, *args, **kwargs):
                super(CustomPolicy, self).__init__(*args, **kwargs,
                                                   net_arch=[128, dict(vf=[128])],
                                                   feature_extraction="mlp")

        register_policy('CustomPolicy', CustomPolicy)

        # model = PPO2(CustomPolicy, env, n_steps=512, nminibatches=4096, noptepochs=15,
        #              verbose=1, tensorboard_log=logPath)
        # model = PPO2(CustomPolicy, env, n_steps=512, nminibatches=4096, noptepochs=10,
        #              verbose=1, tensorboard_log=logPath)
        # model = PPO2(CustomPolicy, env, n_steps=128, nminibatches=4096, gamma=0.98, learning_rate=0.0012311343477785606, ent_coef=0.0019273832114522387, noptepochs=20, lam=0.92, cliprange=0.2,
        #              verbose=1, tensorboard_log=logPath)
        model = PPO2(CustomPolicy, env, n_steps=128, nminibatches=4096, gamma=0.98, learning_rate=0.0012311343477785606, ent_coef=0.0019273832114522387, noptepochs=20, lam=0.92, cliprange=0.2,
                     verbose=1, tensorboard_log=logPath)
        ...
        eval_callback = EvalCallback(eval_env, best_model_save_path=logPath,
                                     n_eval_episodes=10, log_path=logPath,
                                     eval_freq=32000, deterministic=True,
                                     render=False, training_env=env)
        ...
        """
        Best to Worst: Sac3, Sac4, Sac2/1

        Sac1:   'gamma': 0.95, 'lr': 0.0012168610093923691, 'batch_size': 128, 'buffer_size': 1000000, 'learning_starts': 10000, 'train_freq': 300, ent_coef': 0.05, 'net_arch': 'small'
        Sac2:   'gamma': 0.95, 'lr': 0.0004952779670375942, 'batch_size': 512, 'buffer_size': 1000000, 'learning_starts': 10000, 'train_freq': 300, ent_coef': 0.05, 'net_arch': 'small'
        Sac3:   'gamma': 0.9, 'lr': 0.0001368199916822424, 'batch_size': 64, 'buffer_size': 100000, 'learning_starts': 1000, 'train_freq': 1, 'ent_coef': 0.05, 'net_arch': 'small'
        Sac4:   'gamma': 0.9, 'lr': 7.180103359959401e-05, 'batch_size': 64, 'buffer_size': 100000, 'learning_starts': 1000, 'train_freq': 1, 'ent_coef': 0.05, 'net_arch': 'big'. - tester
        """

        # model = SAC(CustomPolicy, env, gamma=0.95,
        #             learning_rate=0.0012168610093923691, buffer_size=1000000,
        #             learning_starts=10000, train_freq=300, batch_size=128,
        #             ent_coef=0.05, verbose=1, tensorboard_log=logPath)
        # model = SAC(CustomPolicy, env, gamma=0.95,
        #             learning_rate=0.0004952779670375942, buffer_size=1000000,
        #             learning_starts=1000, train_freq=1, batch_size=512,
        #             ent_coef=0.05, verbose=1, tensorboard_log=logPath)
        model = SAC(CustomPolicy, env, gamma=0.9,
                    learning_rate=0.0001368199916822424, buffer_size=1000000,
                    learning_starts=1000, train_freq=1, batch_size=64,
                    ent_coef=0.05, verbose=1, tensorboard_log=logPath)
        # model = SAC(CustomPolicy, env, gamma=0.9,
        #             learning_rate=7.180103359959401e-05, buffer_size=1000000,
        #             learning_starts=1000, train_freq=1, batch_size=64,
        #             ent_coef=0.05, verbose=1, tensorboard_log=logPath)
    elif algoName == "TD3":
        eval_callback = EvalCallback(eval_env, best_model_save_path=logPath, n_eval_episodes=10,
    ...
    # model.learn(total_timesteps=1003520, log_interval=10, callback=eval_callback)
    model.learn(total_timesteps=1003520, log_interval=1, callback=eval_callback)
